...
class Template {
	...
	public function addFile($varname, $filename){
		if(!in_array($varname, $this->vars)) throw new InvalidArgumentException("addFile: var $varname não existe");
	...
	public function __set($varname, $value){
		if(!in_array($varname, $this->vars)) throw new RuntimeException("var $varname não existe");
		$stringValue = $value;
		if(is_object($value)){
